...
func createCert(dnsNames []string, parent *x509.Certificate, parentKey crypto.PrivateKey, hoursValid int) (cert []byte, priv []byte) {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"fiber-proxy"},
		},
		DNSNames:  dnsNames,
		NotBefore: time.Now(),
...
func createMitmProxy() *mitmProxy {
	caCert, caKey, err := loadX509KeyPair("./certs/ca.pem", "./certs/ca.key.pem")
	if err != nil {
...
func run() {
	...
	if err := http.ListenAndServe(addr, createMitmProxy()); err != nil {
