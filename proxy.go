...
func Balancer(config Config) fiber.Handler {
	...
	return func(c *fiber.Ctx) error {
		...
		req.SetRequestURI(UnsafeString(req.RequestURI()))
...
// This method will return an fiber.Handler
func Forward(clients ...*fasthttp.Client) fiber.Handler {
	return func(c *fiber.Ctx) error {

		addr := string(c.Request().Header.Peek("Destination"))
...
func doAction(
	...
	copiedURL := CopyString(addr)
	defer req.SetRequestURI(copiedURL)

	log.Printf("Request %s received at %s for URL %s", c.Method(), c.OriginalURL(), copiedURL)
	...
	if scheme := getScheme(UnsafeBytes(copiedURL)); len(scheme) > 0 {
